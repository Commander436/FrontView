...
import {
  ...
  Zap, Radio, Ship, Factory, Landmark,
} from 'lucide-react';
...
const INFRA_CONFIG: { key: keyof LayerVisibility; label: string; icon: any; color: string }[] = [
...
export function LeftPanel({
  ...
  const renderToggle = (key: keyof LayerVisibility, label: string, Icon: any, color: string, indent = false) => (
    <button
      key={key}
      onClick={() => onToggleLayer(key)}
      className={`w-full flex items-center gap-2 px-2 py-1.5 ${indent ? 'pl-8' : ''} rounded text-xs transition-all duration-200 ${
        layers[key]
          ? indent ? color : 'bg-secondary/40 text-foreground'
          : 'text-muted-foreground hover:bg-secondary/20'
      }`}
    >
      <Icon className={`w-3.5 h-3.5 ${layers[key] ? (indent ? '' : color) : 'opacity-30'}`} />
      <span className="font-display tracking-wider text-[9px]">{label}</span>
      {!indent && layers[key] && getCount(key) !== undefined && (
        <span className="ml-auto text-[8px] font-mono text-primary">{getCount(key)}</span>
      )}
  ...
  return (
    <aside className={`${collapsed ? 'w-12' : 'w-64'} transition-all duration-300 backdrop-blur-2xl bg-card/60 border-r border-border/30 flex flex-col relative z-40 shrink-0`}>
      <button
        onClick={onToggleCollapse}
        className="absolute -right-3 top-4 w-6 h-6 bg-card/80 backdrop-blur-lg border border-border/30 rounded-full flex items-center justify-center text-muted-foreground hover:text-primary transition-colors z-50"
      >
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-secondary/60 backdrop-blur-lg border border-border/30 rounded-lg px-2 py-1.5 text-[10px] font-display tracking-wider text-foreground focus:outline-none focus:ring-1 focus:ring-primary/50 cursor-pointer"
            >
              {DISPLAY_MODES.map(m => (
          ...
          <div>
            ...
            <div className="space-y-0.5">
              {LAYER_CONFIG.map(({ key, label, icon: Icon, color, subToggles }) => (
                <div key={key}>
                  {renderToggle(key, label, Icon, color)}
                  {subToggles && layers[key] && subToggles.map(sub =>
                    renderToggle(sub.key, sub.label, sub.icon, sub.color, true)
                  )}
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="space-y-0.5">
              {INFRA_CONFIG.map(({ key, label, icon: Icon, color }) =>
                renderToggle(key, label, Icon, color)
              )}
            </div>
          </div>
      ...
      {collapsed && (
        <div className="flex-1 flex flex-col items-center pt-6 gap-2">
          ...
          {INFRA_CONFIG.map(({ key, icon: Icon, color }) => (
            <button
              key={key}
              onClick={() => onToggleLayer(key)}
              className={`p-1.5 rounded transition-colors ${layers[key] ? color : 'text-muted-foreground/30'}`}
